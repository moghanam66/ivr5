...
async def recognize_speech_async():
    """Recognize speech continuously using async."""
    ...
    def recognized_cb(evt: speechsdk.SpeechRecognitionEventArgs):
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            asyncio.create_task(recognized_queue.put(evt.result.text))
    ...
    try:
        while not stop_future.done():
            user_query = await recognized_queue.get()
            if user_query:
                yield user_query
    finally:
...
async def speak_response_async(text: str):
    ...
    if result.reason == speechsdk.ResultReason.Canceled:
        ...
        logging.error("Speech synthesis canceled: %s", cancellation.error_details)
...
# Voice Chat Processing Logic
# ------------------------------------------------------------------
class VoiceChatBot:
    def __init__(self):
        self.active = True

    async def voice_processing_loop(self):
        """Main loop handling voice interaction."""
        async for user_query in recognize_speech_async():
            if not self.active:
                break
            
            cleaned_query = clean_text(user_query)
            if cleaned_query in ["إنهاء", "خروج"]:
                await speak_response_async("مع السلامة، نراكم قريبًا!")
                self.active = False
                break
            
            if detect_critical_issue(user_query):
                text="هذه المشكلة تحتاج إلى تدخل بشري. سأقوم بالاتصال بخدمة العملاء لدعمك."
                await speak_response_async(text)
                ssml_response = f"""<speak version='1.0' xmlns='https://www.w3.org/2001/10/synthesis' xml:lang='ar-EG'>
                              <voice name='ar-EG-ShakirNeural'>{text}</voice>
                              </speak>"""
                escalation_context = {
                        "tags": {
                            "type": "Escalate",
            ...
            # Process query through existing NLP pipeline
            response = await get_response(user_query)
            ssml_response = f"""<speak version='1.0' xmlns='https://www.w3.org/2001/10/synthesis' xml:lang='ar-EG'>
                              <voice name='ar-EG-ShakirNeural'>{response}</voice>
                              </speak>"""  
            await speak_response_async(response)
            return Activity(
        type=ActivityTypes.message,
        text=response,
        speak=ssml_response,
...
class MyBot(ActivityHandler):
    def __init__(self):
        self.voice_bot = VoiceChatBot()

    async def on_message_activity(self, turn_context: TurnContext):
        #user_query = turn_context.activity.text
        response_text =  self.voice_bot.voice_processing_loop()
    ...
    async def on_members_added_activity(self, members_added: ChannelAccount, turn_context: TurnContext):
        """Start voice processing when user joins."""
        for member in members_added:
            if member.id != turn_context.activity.recipient.id:
                await speak_response_async("مرحبًا! أنا مساعدك الصوتي، كيف يمكنني مساعدتك اليوم؟")
                ...
                await turn_context.send_activity(welcome_activity,speak=welcome_activity)
                # Start voice processing in background
                asyncio.create_task(self.voice_bot.voice_processing_loop())
